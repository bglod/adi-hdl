...
if {[info exists ::env(ADI_PROJECT_DIR)]} {
  set mem_init_sys_file_path [file join [pwd] $::env(ADI_PROJECT_DIR)]
}
...
proc sysid_gen_sys_init_file {{custom_string {}} {address_bits {9}}} {
  ...
  if {[info exists ::env(ADI_PROJECT_DIR)]} {
    set mem_init_sys_file_path [file join [pwd] $::env(ADI_PROJECT_DIR) "mem_init_sys.txt"]
  } else {
...
proc sysid_gen_pr_init_file {custom_string} {
  ...
  if {[info exists ::env(ADI_PROJECT_DIR)]} {
    set mem_init_pr_file_path [file join $::env(ADI_PROJECT_DIR) "mem_init_pr.txt"]
  } else {
